...
𝒜memEqPreVar {zero} {zero} {mₜ = mₜ} Skipₜ _ _              | yes lAz=lA'z = 
  cong (λ x → lookupOrDefault x (lookup mₜ zero)) (sym lAz=lA'z)
...
𝒜memEqPreVar {suc currVar'} {varName} {cV<n} {A} {A'} {mₜ} {mₜ'} (Seqₜ Skipₜ d) _ _ | yes vN=cV | yes lAcV=lA'cV = 
  let lmₜvN=lmₜ'vN : lookup mₜ varName ≡ lookup mₜ' varName
      ...
      lAvN=lA'vN : lookup A varName ≡ lookup A' varName
      lAvN=lA'vN = subst (λ x → lookup A x ≡ lookup A' x) (sym vN=cV) lAcV=lA'cV
   in cong₂ (λ x y → lookupOrDefault x y) (sym lAvN=lA'vN) lmₜvN=lmₜ'vN
𝒜memEqPreVar {suc currVar'} {varName} {cV<n} {A} {A'} {mₜ} {mₜ'} (Seqₜ {.mₜ} {mₜ1} {.mₜ'} Assignₜ d) _ wFmₜA | yes vN=cV | no _ = 
  let finCurrVar = fromℕ< cV<n
      ...
      lmₜ1AcV=lmₜA'cV : lookupₜ mₜ1 A finCurrVar ≡ lookupₜ mₜ A' finCurrVar
      lmₜ1AcV=lmₜA'cV = lookupₜx∘changeₜx {n} {_} {lookup A finCurrVar} finCurrVar mₜ (wFmₜA finCurrVar)
      cV=vN : fromℕ< cV<n ≡ varName
      cV=vN = sym (toℕ-injective (trans vN=cV (sym (toℕ-fromℕ< cV<n))))
      lmₜA'vN=lmₜ1AvN : lookupₜ mₜ A' varName ≡ lookupₜ mₜ1 A varName
      lmₜA'vN=lmₜ1AvN = subst (λ x → lookupₜ mₜ A' x ≡ lookupₜ mₜ1 A x) cV=vN (sym lmₜ1AcV=lmₜA'cV)
   in subst (λ x → lookupₜ mₜ A' varName ≡ lookupOrDefault (lookup A varName) x) lmₜ1vN=lmₜ'vN lmₜA'vN=lmₜ1AvN
...
𝒜memEqPreVar currVar@{suc _} {varName} {cV<n} {A} {A'} {mₜ} {mₜ'} (Seqₜ {.mₜ} {mₜ1} {.mₜ'} assign@Assignₜ d) vN≤cV wFmₜA | no vN<>cV | no _ =
  let vN<>fromN<cV : varName ≢  fromℕ< cV<n
      ...
      lmₜ1A'vN=lmₜA'vN : lookupₜ mₜ1 A' varName ≡ lookupₜ mₜ A' varName
      lmₜ1A'vN=lmₜA'vN = lookupₜy∘changeₜx (fromℕ< cV<n) varName mₜ vN<>fromN<cV
      lmₜ1A'vN=lmₜ'AvN : lookupₜ mₜ1 A' varName ≡ lookupₜ mₜ' A varName
      lmₜ1A'vN=lmₜ'AvN = 𝒜memEqPreVar d (m<1+n⇒m≤n (≤∧≢⇒< vN≤cV vN<>cV)) (wellFormed-trans {_} {_} {_} {A} wFmₜA assign)
   in trans (sym lmₜ1A'vN=lmₜA'vN) lmₜ1A'vN=lmₜ'AvN
